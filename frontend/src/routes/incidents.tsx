      ...
      <SectionHeader
        ...
        actions={
          <div className="flex items-center gap-1 rounded-md border border-border bg-muted/40 p-0.5">
            {filters.map((f) => (
              <button key={f} onClick={() => setFilter(f)} className={`rounded px-2.5 py-1 font-mono text-[10px] uppercase tracking-widest ${filter === f ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}>{f}</button>
            ))}
          </div>
        }
      />
            ...
            <ul className="space-y-5">
              {rows.map((inc) => (
                <li key={inc.id} className="relative grid grid-cols-[5rem_auto_1fr_auto] items-start gap-4">
                  ...
                  <span className="mt-1.5 h-3 w-3 rounded-full ring-2 ring-background"
